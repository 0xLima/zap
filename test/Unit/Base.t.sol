...
contract Demo is Test {
    ...
    function setUp() public {
        ...
        mainRouter.setBinanceDepositor(address(depositor));
        mainRouter.setBinanceMinter(address(minter));
    ...
    modifier DepositAndMint (ERC20Mock _token, uint256 _amountDeposit, uint256 _amountMint) {
        ...
        assert(_amountMint == zxp.balanceOf(user));
    ...
    function test_mint() external Deposit(weth, 1 ether) {
        ...
        console.log("Balance of user minted before minting: ", zxp.balanceOf(user));
        ...
        console.log("Balance of user minted after minting: ", zxp.balanceOf(user));
    ...
    function test_burn() external DepositAndMint(weth, 1 ether, 1000 ether){
        ...
        vm.startPrank(user);
        zxp.approve(address(minter), _amountBurn);
        minter.burn(_amountBurn);
        vm.stopPrank();

        console.log("Balance of user minted after burning: ", zxp.balanceOf(user));

        assert(_amountMint - _amountBurn == zxp.balanceOf(user));
    ...
    function testBurnAndMint() external DepositAndMint(weth, 1 ether, 1000 ether) {
        vm.startPrank(user);
        zxp.approve(address(minter), 500 ether);
    ...
    function testLiquidate() external DepositAndMint(weth, 1 ether, 1000 ether) {
        ...
        assert(_amountMint == zxp.balanceOf(liquidator));
        ...
        vm.startPrank(liquidator);
        zxp.approve(address(minter), 1000 ether);
