...
contract Minter is CCIPBase {
    ...
    function burn(uint256 _amount) external payable {
        ...
        IERC20(address(zxp)).safeTransferFrom(msg.sender, address(this), _amount);
        zxp.burn(_amount);
    ...
    function liquidate(
        ...
        feePay[msg.sender] += msg.value;
        IERC20(address(zxp)).safeTransferFrom(msg.sender, address(this), _amountToCover);
        zxp.burn(_amountToCover);
    ...
    function getZxp() public view returns (ZXP) {
        return zxp;
    }
