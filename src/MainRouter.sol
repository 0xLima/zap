...
contract MainRouter is CCIPBase, FunctionsBase {
    ...
    address private binanceDepositor;
    address private binanceMinter;
    ...
    modifier onlyBinanceDepositor(address _caller) {
        if (binanceDepositor != _caller){
            revert NotAllowed();
        }
        _;
    }

    modifier onlyBinanceMinter(address _caller) {
        if (binanceMinter != _caller){
            revert NotAllowed();
        }
        _;
    }

    function setBinanceDepositor(address _depositorContract) external onlyOwner{
        binanceDepositor = _depositorContract;
    }

    function setBinanceMinter(address _minterContract) external onlyOwner {
        binanceMinter = _minterContract;
    }
    ...
    )   external onlyBinanceDepositor(msg.sender) {
    ...
    function burnAndMint(
        ...
        uint64 _destinationChainSelector,
        address _receiver
    ) external onlyBinanceMinter(msg.sender) {
        _burn(_burner, _chainSelector, _amount);
        _mint(_destinationChainSelector, _receiver, _burner, _amount);
    }

    function deposit(address _depositor, uint64 _sourceChainSelector, address _token, uint256 _amount) external onlyBinanceDepositor(msg.sender) {
        _deposit(_depositor, _sourceChainSelector, _token, _amount);
    }

    function burn(address _burner, uint64 _sourceChainSelector, uint256 _amount) external onlyBinanceMinter(msg.sender) {
    ...
    function getBinanceDepositor() public view returns (address) {
        return binanceDepositor;
    }

    function getBinanceMinter() public view returns (address) {
        return binanceMinter;
    }
